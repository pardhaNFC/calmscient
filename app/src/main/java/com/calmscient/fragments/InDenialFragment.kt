...
import com.calmscient.adapters.InDenialAdapter
import com.calmscient.adapters.PushThoughAdapter
import com.calmscient.databinding.FragmentInDenialBinding
...
class InDenialFragment : Fragment() {
    private lateinit var binding: FragmentInDenialBinding
    private lateinit var anxietyAdapter: InDenialAdapter
    ...
    override fun onCreateView(
        ...
        binding = FragmentInDenialBinding.inflate(inflater, container, false)
        val view = binding.root

        val pagerSnapHelper = PagerSnapHelper()
        pagerSnapHelper.attachToRecyclerView(binding.inDenialRecyclerView)
    ...
    private fun initializeAdapter() {
        binding.inDenialRecyclerView.layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
        anxietyAdapter = InDenialAdapter(requireContext(),anxietyText)
        binding.inDenialRecyclerView.adapter = anxietyAdapter
    }
    ...
    private fun displayMakeAPlanViews() {
        anxietyText.add(
            ChangingResponseLessonThreeDataClass(
                getString(R.string.in_denial),
                null,
                null,
                null,
                null,
                null,
                R.drawable.in_denial_1,
                getString(R.string.in_denial_card_1_desc_3),
            )
        );
        anxietyText.add(
            ChangingResponseLessonThreeDataClass(
                ...
                getString(R.string.in_denial_card_2_desc_1),
                getString(R.string.feelings_enotions_triggered_by_these_thoughts),
                getString(R.string.in_denial_card_2_desc_2),
                getString(R.string.behavioural_responses),
                null,
                getString(R.string.in_denial__card_2_desc_3),
            )
        );
        anxietyText.add(
            ChangingResponseLessonThreeDataClass(
                getString(R.string.in_denial_card_3_heading_1),
                null,
                null,
                null,
                null,
                null,
                null,
                getString(R.string.in_denial_card_3_desc_3),
            )
        );
    ...
    private fun setupNavigation() {
        ...
        binding.inDenialRecyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
    ...
    private fun navigateToQuestion(index: Int, isNext: Boolean) {
        if (index in 0 until anxietyText.size) {
            ...
            binding.inDenialRecyclerView.smoothScrollToPosition(currentQuestionIndex)
            // Calculate and set the progress based on the current question index
            progressBar.progress = currentQuestionIndex * (maxProgress / (anxietyText.size - 1))
            // Update the step indicators (ImageViews) as active or inactive
            updateStepIndicators()
