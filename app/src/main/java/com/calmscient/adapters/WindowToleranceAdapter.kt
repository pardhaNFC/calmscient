...
class WindowToleranceAdapter(
    private val context: Context,
    val items: MutableList<EmotionalSignsOFStressDataClass>
) :
    RecyclerView.Adapter<WindowToleranceAdapter.WindowToleranceHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): WindowToleranceHolder {
        val itemView = LayoutInflater.from(parent.context)
            .inflate(R.layout.emotional_signs_of_stress_itemcard, parent, false)
        return WindowToleranceHolder(itemView)
    }

    override fun onBindViewHolder(holder: WindowToleranceHolder, position: Int) {
    ...
    inner class WindowToleranceHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
