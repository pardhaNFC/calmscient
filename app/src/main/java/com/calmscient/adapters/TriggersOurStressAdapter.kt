...
class TriggersOurStressAdapter(
    private val context: Context,
    val items: MutableList<EmotionalSignsOFStressDataClass>
) :
    RecyclerView.Adapter<TriggersOurStressAdapter.TriggersOurStressHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): TriggersOurStressHolder {
        val itemView = LayoutInflater.from(parent.context)
            .inflate(R.layout.emotional_signs_of_stress_itemcard, parent, false)
        return TriggersOurStressHolder(itemView)
    }

    override fun onBindViewHolder(holder: TriggersOurStressHolder, position: Int) {
    ...
    inner class TriggersOurStressHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
