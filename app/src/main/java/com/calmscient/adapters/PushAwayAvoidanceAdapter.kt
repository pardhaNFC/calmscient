...
class PushAwayAvoidanceAdapter(
    private val context: Context,
    val items: MutableList<ChangingResponseLessonThreeDataClass>
) :
    RecyclerView.Adapter<PushAwayAvoidanceAdapter.PushAwayAvoidanceHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): PushAwayAvoidanceHolder {
        val itemView = LayoutInflater.from(parent.context)
            .inflate(R.layout.changing_response_lesson_three_itemcard, parent, false)
        return PushAwayAvoidanceHolder(itemView)
    }



    override fun onBindViewHolder(holder: PushAwayAvoidanceHolder, position: Int) {
    ...
    inner class PushAwayAvoidanceHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
